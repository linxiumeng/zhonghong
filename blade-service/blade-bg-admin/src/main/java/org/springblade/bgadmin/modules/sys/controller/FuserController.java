...
import io.finepetro.modules.sys.form.UserForm;
...
public class FuserController {
    ...
    public R info(@PathVariable("userId") Integer userId) {
            FuserEntity fuser = fuserService.selectById(userId);
    ...
    public R update(@RequestBody FuserEntity fuser) {
        ValidatorUtils.validateEntity(fuser);
            fuserService.updateAllColumnById(fuser);//全部更新
    ...
    public R delete(@RequestBody Integer[] userIds) {
            fuserService.deleteBatchIds(Arrays.asList(userIds));
    ...
    public R creditUserById(@RequestBody UserForm userForm){
        ...
        AccountEntity accountEntity = accountService.selectOne(new EntityWrapper<AccountEntity>().eq("user_id",userId));
        ...
        boolean flag = fuserService.updateById(fuserEntity);
        flag = accountService.update(accountEntity,new EntityWrapper<AccountEntity>().eq("user_id",userId));
        if(flag){
            return R.ok();
        }
        return R.error("操作失败请联系管理员");
